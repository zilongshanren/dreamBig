...
2. 字段名**必须**使用下方骨架里给出的英文 key（不要翻译成中文，不要
   改 key 名，不要合并字段）。字段**值**必须用简体中文。
...
9. **data_blind_spots 至少 1 条**。每条必须同时包含 signal / reason / impact
   三个 key。没有盲区 = 你在撒谎 = 置信度归零。

## 字段类型硬约束（前一次失败根因 — 严格遵守）

- `market_pulse` **只能**是这五个英文字符串之一：
  `"hot"` / `"warming"` / `"stable"` / `"cooling"` / `"cold"`。
  **不要**写整句中文描述代替枚举值。
- `red_flags[].affected_games` 是**纯整数数组**（例如 `[608, 625]`），
  **不要**写 `["game:608"]` 这种带前缀的字符串。
- `project_recommendations[].inspirations` 同上，是纯整数数组。
- `signal_games[].evidence_refs` 是字符串数组（可以写 `"game:123"` 或
  `"chart:top_grossing"`）。和上面 affected_games 不是一回事。
- `top_signal_games[].game_id` / `project_recommendations[].*` 不要
  翻译 key 名——保持英文。

## 标准 JSON 骨架（字段名 **一个字符都不能改**）

```json
{
  "headline": "字符串",
  "market_pulse": "warming",
  "market_snapshot": "字符串",
  "top_signal_games": [
    {
      "game_id": 123,
      ...
      "signal_strength": "字符串",
      "iaa_angle": "字符串",
      "evidence_refs": ["chart:hot", "game:123"]
    }
  ],
  ...
  "project_recommendations": [
    {
      ...
      "genre": "puzzle",
      "core_mechanic": "字符串",
      "inspirations": [123, 456],
      "iaa_placement_hint": "字符串",
      "rationale": "字符串",
      "target_audience": "字符串",
...
遇到字段名时**直接复制上面的英文 key**，不要即兴改造。上一次失败就是因为
把 `pattern` 写成 `flag`、把 `signal/reason/impact` 合并成单个 `blind_spot`、
把 `market_pulse` 写成了整段中文——这三种错都会被 Pydantic 直接拒收。
"""
